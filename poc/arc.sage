...
class PresentationState(object):
    ...
    def present(self, rng, vectors):
        ...
        vectors["a"] = to_hex(G.ScalarField.serialize([a]))
        vectors["r"] = to_hex(G.ScalarField.serialize([r]))
        vectors["z"] = to_hex(G.ScalarField.serialize([z]))
...
class Client(object):
    ...
    def request(self, request_context, vectors):
        ...
        vectors["m1"] = to_hex(G.ScalarField.serialize([m1]))
        vectors["m2"] = to_hex(G.ScalarField.serialize([m2]))
        vectors["r1"] = to_hex(G.ScalarField.serialize([r1]))
        vectors["r2"] = to_hex(G.ScalarField.serialize([r2]))
        vectors["m1_enc"] = to_hex(G.serialize([m1_enc]))
        vectors["m2_enc"] = to_hex(G.serialize([m2_enc]))
        vectors["proof"] = to_hex(proof)
...
class Server(object):
    ...
    def keygen(cls, rng, vectors):
        ...
        vectors["x2"] = to_hex(G.ScalarField.serialize([x2]))
        vectors["xb"] = to_hex(G.ScalarField.serialize([xb]))
